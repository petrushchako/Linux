...
## edit `DocumentRoot` with new value `/web_content`
## Navigate to <Directory> tg, press 5 (to copy five lines) and press `y` twice
## Navigate past cloding </Directory> tag and pre `p` to paste compied content
## Replace value within <Directory> of the copied content to `/web_content`
## :wq (to save and exit)
```
...
ll -dZ /web_content/
## Make adjustments to context, to make it similar of `ll -dZ /var/www/html/`
...
## populate file with "Just a test!"
## Copy the public ip address (not the result of `'ip a`) and use it in the browser to view content of index.html (http://<pub_ip_address>)
```