...
Note, there are many tutorial documents, web pages and videos available on the Internet if you desire to learn a lot more about using git for version controlling your files; using your favorite search engine to search for "git" or "git tutorial" will reveal many resources to explore. In this course, we are just going to touch on the very basics.

The first thing you may want to do is to configure some global configuration values:
```bash
$ git config --global user.name "my-username" (where my-username is your name)
$ git config --global user.email "user@email.com" (where you supply your email address)
```
Next, set up a local working directory. Let’s say you have a project called `my-app` that you want to track with version control. To set up the working directory, create a new subdirectory in your home (or login) directory called `my-app` and initialize this for git:
```bash
$ cd
$ mkdir my-app
$ cd my-app
...
The directory `~/my-app` is now initialized with git to be a git working directory. Start by editing a file, `project-plan`, and have it contain anything you want; it can even be gibberish if you want. In this example, we will use a terminal text editor called vim as our editor of choice. Vim will automatically create and open the file. To edit the file, press "i" on the keyboard, which will allow you to enter "insert" mode. When you are done editing, press "esc". Type `":wq"`. This will save the file then quit vim.
```
$ vim project-plan      # use the editor to create lines in the file
$ cat project-plan
This is my project plan.
It has three lines in it.
I need to add more information later.
$ git status
$ git add project-plan
$ git commit
$ git status
$ vim project-plan   # use the editor to add a couple of more lines to the file
$ cat project-plan
This is my project plan.
It has three lines in it.
I need to add more information later.
I added a couple of more lines in the file.
These might represent more detail in my project plan.
$ git status
$ git add project-plan
$ git commit
$ git log
...
In this example case, we are only using a local repository. Some of the git commands are just to show you the status of the file, working directory, staging area, and local repository. The two main git commands that were used above are:

- `git add` is a command used to add a file that is in the working directory to the staging area.
- `git commit` is a command used to add all files that are staged to the local repository.

At this point, let’s assume that there are a few other project team members who want to make changes to the `project-plan` file. Git allows multiple people to work on the same file. Each person creates their own branch (with the `git branch` command) and then switches to that branch (with the `git checkout` command). The default branch is called `main`. Let’s say that this project’s Technical Lead wanted to make some changes to the `project-plan` file, but knew that the Project Manager was also actively working on that file. In order to not get in the way of the Project Manager, the Technical Lead creates a branch in order to make changes to the `project-plan` file:

```bash
$ git branch tech-lead         # create a new branch called tech-lead
$ git branch --list            # list all current branches
$ git checkout tech-lead       # switch to the tech-lead branch
$ vim project-plan             # Tech Lead modifies file
$ git add project-plan         # just like before, but now changes...
$ git commit project-plan.     # ... are one the tech-lead branch
```
A shortcut to create and switch to a branch can be done by running:

```bash
$ git checkout -b tech-lead # this is a one-step way to create and
                            # switch to the tech-lead branch
```

Later on, the Technical Lead could then `merge` the changes made on the **tech-lead branch** back into the **main branch** so that all modifications are together in one version of the file:
...
During the merge operation, git tries to automatically reconcile the differences between the versions of affected files. In some cases, the person running the `git merge` command may need to manually edit the file in order to merge all the changes into the affected files. At this point, the Technical Lead may decide that the **tech-lead branch** is no longer needed and delete the branch:
```bash
$ git branch -d tech-lead    # -d option deletes the branch
```

![](img/Cpt10.6.png)

**Git Trunk and Branch Flow**

As noted earlier, there is a lot more to using git, but this shows you the idea of how you can start to track changes that you make on files. For software development, those files are often programming code. Systems Administrators may track configuration file changes. You may also want to track changes in text files like we did with the `project-plan` file above.

<br>

### Git Best Practices

Some git best practices include:

- Commit changes to code/text often so that small changes are trackable (you can back-out a feature if needed) and explainable in your log messages (explaining details of hundreds of lines of code is near impossible).
- Make detailed notes in the commit log! Remember, you may not be the person who next maintains the changes you made to a file; even if you are, a year or more may have passed since you last looked at these changes. Detailed notes should explain why you made changes; someone else should be able to understand why you changed something. You are likely working with other people who need to understand your changes.
- Ensure you are working from the latest version of the code or text; otherwise, you may miss and lose changes made by others in this collaborative environment.
- Review changes before committing. Too many correcting commits tend to muddy the waters. Someone, or even yourself, looking over a changelog months later may only be confused by needless changes that could have been avoided by review.
- Use branches to allow and encourage collaboration with other developers and admins working on the same file and implementing new features and fixes. This should promote efficient progress since you don’t need to wait for each other to finish work on the same set of files; you can all do work in parallel.