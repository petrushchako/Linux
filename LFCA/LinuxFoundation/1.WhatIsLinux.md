...
- Types of Computers
    ...
    - **Servers**
        ...
        Servers serve content to other computers and client users, and are the backbone of how the modern cloud computing environment works today. Whether you are using email, messaging, streaming videos, or a web browser, your data is being served by a server. Servers are generally "workhorses" low on frills and high on compute power and/or storage. Often, the main background engine for an application is run on a machine with no display, keyboard or mouse, just a machine connected to the network doing one or two jobs to the absolute best performance it can. Sometimes, the job a server must do is small, like a weather recording station, where the "server" is a tiny machine recording data every 15 seconds and uploading that data to another server. Some Linux distributions tailor their Linux installation to favor the common need of the server environment. Such examples include Red Hat Enterprise Linux, Ubuntu Server, Debian, openSUSE Leap and Fedora Server. Servers provide services; these services need to be managed. Managing services is an important role for a system administrator.

    - **Close Desktops**

        Desktop computers usually have monitors, keyboards and mice attached to them, as the user is typically using the machine for business applications like email, and custom applications relevant to the business (like "point of sale" or "finance" applications). Desktops are also known to be medium to extravagant systems for photo, video or gaming applications. Some of the largest single footprint machines are used for gaming.

    - **Close Embedded**

        Small purpose built computers are everywhere: cars, refrigerators, TVs and almost anything you can imagine. Many of these special purpose built systems run on highly customized versions of Linux. These are known as embedded systems.
